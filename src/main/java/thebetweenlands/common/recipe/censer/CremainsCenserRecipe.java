	...
	public int update(Void context, Censer censer) {
		...
		if(level instanceof ServerLevel serverLevel && level.getGameTime() % 10 == 0) {
			...
			for(LivingEntity living : affected) {
				if(!living.isInWater() && !living.isInvulnerableTo(serverLevel, level.damageSources().inFire()) && !living.fireImmune() && (!(living instanceof Player player) || !player.isCreative())) {
